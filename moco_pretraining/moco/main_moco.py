...
def main_worker(gpu, ngpus_per_node, args, checkpoint_folder):
    ...
    # # # save model in onxx format
    # # d1 = torch.rand(1, 3, 224, 224)
    # # d2 = torch.rand(1, 1)
    # input_names = ["input0", "input1"]
    # output_names = ["output0","output1"]
    # inputs = (torch.randn(1, 3, 320, 320).cuda(), torch.randn(1, 3, 224, 224).cuda())
    # # m = model()
    # # o = m(d)
    # model.eval()
    # # o = model(d1, d2)
    # onnx_path = os.path.join(checkpoint_folder, args.arch + "_model.onnx")
    # # torch.onnx.export(m, d, onnx_path)
    # # torch_out = torch.onnx._export(model.module, inputs, onnx_path, export_params=True, verbose=True,
    # #                                 input_names=input_names, output_names=output_names)
    # torch_out = torch.onnx._export(model.module, inputs, onnx_path, export_params=True, verbose=True, opset_version=11)
