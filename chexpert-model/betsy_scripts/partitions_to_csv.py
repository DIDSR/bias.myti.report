...
partition_base_dir = "/scratch/alexis.burgon/2022_CXR/model_runs/20220902"
partition_folders = [f"RAND_{i}_OPTION_0" for i in range(2)]

orig_summary_folder = "/scratch/alexis.burgon/2022_CXR/data_summarization/20220823"
tasks = {
    'patient_sex' :['M', 'F'],
            ...
            for ii, row in in_df.iterrows():
                ...
                for key, val in conv_tables.items():
                    if repo_name_adj[row['repo']] in key:
                        jpeg_dict = val
                if not jpeg_dict:
                    print(f"Couldn't load jpeg dict for {repo_name_adj[row['repo']]}")
                    continue
